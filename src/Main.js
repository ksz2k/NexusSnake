const {
  libraries: { React },
  components: { GlobalStyles, Panel, Button },
} = NEXUS;
...
class Snake extends React.Component {
  ...
  moveFood() {
    ...
    this.moveFoodTimeout = setTimeout(this.moveFood, 75000 / this.cellSize)
  }
  ...
  startGame() {
    this.removeTimers();
    this.moveSnakeInterval = setInterval(this.moveSnake, this.cellSize * 10);
    this.moveFood();
    ...
    this.setState({
      ...
      snake: [
        [
          parseInt(Math.random() * Math.floor(this.props.size / this.cellSize)),
          parseInt(Math.random() * Math.floor(this.props.size / this.cellSize))
        ]
      ],
      food: [
      ...
      direction: this.directions[parseInt(Math.random() * 4)]
    });
    this.el.focus();
  }
  ...
  render() {
    this.numCells = Math.floor(this.props.size / this.cellSize);
...
const App = () => (
  <Panel title="Snake" icon={{ url: 'images/icon.png', id: 'icon' }}>
    <GlobalStyles />
    <div class="mt1">
     <Snake size={350} />
    </div>
